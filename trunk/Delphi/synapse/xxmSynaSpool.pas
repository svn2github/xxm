...
uses SysUtils, blcksock, synsock, xxmThreadPool, xxmCommonUtils, xxmSynaMain;
          ...
          if FContexts[j].Context<>nil then
           begin
            ...
            if (FContexts[j].KeptCount=300) or
              ((FContexts[j].Context as TXxmSynaContext).Socket=nil) then
              DropContext(j)
            else
             begin
              h:=(FContexts[j].Context as TXxmSynaContext).Socket.Socket;
            ...
            for k:=0 to x.fd_count-1 do
             begin
              ...
              while (j<FContextIndex) and not((FContexts[j].Context<>nil)
                and ((FContexts[j].Context as TXxmSynaContext).Socket.Socket=h))
                do inc(j);
             ...
             begin
              ...
              if j<FContextIndex then
               begin
                ...
                if (l=0) or ((FContexts[j].Context as TXxmSynaContext).
                  Socket.SendBuffer(@d[0],l)<>l) then
                  DropContext(j)//raise?
                else
                 begin
